...
const AlignEngine = {
  ...
  _fallback(dir) {
    ...
    if (dir === 'left') { const minX = Math.min(...els.map(e => e.x)); els.forEach(e => DS.updateElementLayout(e.id, { x: minX }, 'AlignEngine.fallback')); }
    if (dir === 'right') { const maxR = Math.max(...els.map(e => e.x + e.w)); els.forEach(e => DS.updateElementLayout(e.id, { x: maxR - e.w }, 'AlignEngine.fallback')); }
    if (dir === 'top') { const minY = Math.min(...els.map(e => e.y)); els.forEach(e => DS.updateElementLayout(e.id, { y: minY }, 'AlignEngine.fallback')); }
    if (dir === 'bottom') { const maxB = Math.max(...els.map(e => e.y + e.h)); els.forEach(e => DS.updateElementLayout(e.id, { y: maxB - e.h }, 'AlignEngine.fallback')); }
