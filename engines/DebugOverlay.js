...
const _DEBUG_OVERLAY_CSS = `:host{position:fixed;left:10px;bottom:26px;right:auto;top:auto;z-index:9010;min-inline-size:180px;max-inline-size:220px;max-block-size:min(52dvh,calc(100dvb - 40px));padding:8px 10px;display:none;flex-direction:column;gap:6px;background:rgba(9,18,14,.9);border:1px solid rgba(82,255,122,.4);border-radius:8px;box-shadow:0 8px 20px rgba(0,0,0,.35);color:#d7ffe1;font:11px/1.2 system-ui,sans-serif;pointer-events:auto;user-select:none;-webkit-user-select:none;backdrop-filter:blur(4px);box-sizing:border-box;overflow:hidden}:host(.is-on){display:flex}#rf-debug-overlay-head{display:flex;justify-content:space-between;align-items:center;gap:8px;color:#9dffb2;text-transform:uppercase;letter-spacing:.06em;font-size:9px;cursor:grab}#rf-debug-overlay-head.is-dragging{cursor:grabbing}#rf-debug-overlay-head button{appearance:none;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);color:inherit;border-radius:999px;padding:2px 6px;font:inherit;cursor:pointer}#rf-debug-overlay-layers{display:grid;grid-template-columns:1fr;gap:4px;max-block-size:calc(52dvh - 44px);overflow:auto}:host([data-collapsed="true"]) #rf-debug-overlay-layers{display:none}.rf-debug-layer{display:grid;grid-template-columns:1fr auto;gap:6px;align-items:center;padding:4px 6px;border-radius:5px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.05);transition:background .12s ease,border-color .12s ease,box-shadow .12s ease}.rf-debug-layer.active{background:rgba(82,255,122,.14);border-color:rgba(82,255,122,.45);box-shadow:inset 0 0 0 1px rgba(82,255,122,.18),0 0 10px rgba(82,255,122,.12)}.rf-debug-layer-name{display:flex;align-items:center;gap:6px;min-width:0}.rf-debug-layer-dot{inline-size:7px;block-size:7px;border-radius:50%;background:rgba(164,172,168,.55);flex:0 0 auto}.rf-debug-layer.active .rf-debug-layer-dot{background:#52ff7a;box-shadow:0 0 8px rgba(82,255,122,.75)}.rf-debug-layer-label{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.rf-debug-layer-meta{color:rgba(215,255,225,.72);font-size:10px}`;
