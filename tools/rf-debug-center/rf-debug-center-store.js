...
import { clone, compareSnapshots, normalizeSeverity, normalizeSnapshot, safeObject, stringify } from './rf-debug-center-state-utils.js'; import { clearDomScannerSnapshot as clearDomScannerSnapshotCore, copyDomScannerJSON as copyDomScannerJSONCore, getDomScannerSnapshot as getDomScannerSnapshotCore, isDomScannerPaused as isDomScannerPausedCore, refreshDomScannerSnapshot as refreshDomScannerSnapshotCore } from './rf-debug-center-dom-scanner.js';
const MAX_RECENT = 12;
const timelineState = {
  paused: false,
  cursor: 0,
...
export function readDebugCenterState({ traceApi = window.RF_UI_TRACE, enabled = false, activation = 'disabled', bundle = null } = {}) {
  ...
  if (!timelineState.warningsPaused) timelineState.warnings = buildWarningsSnapshot({ traceState: timeline.sourceState, timeline, zoom, renderPreview, asyncRace, loopFreeze, network, performance, selection, visualEvidence: getVisualEvidenceSnapshot(), dom: { status: zoom.status || 'unknown', findings: zoom.divergences?.map((code) => ({ code })) || [], owners: zoom.suggestedOwner ? [zoom.suggestedOwner] : [] }, domScanner, bundle, ownership: window.RFDebugCenter?.ownership || null, active: Boolean(enabled) });
  return { enabled: Boolean(enabled), activation, build, debugZoom, last, live: normalizeSnapshot(live), divergence, zoom, timeline, selection, renderPreview, domScanner, asyncRace, loopFreeze, network, performance, warnings: timelineState.warnings, recent: timeline.recent, visualEvidence: getVisualEvidenceSnapshot() };
}
