...
import { clone, compareSnapshots, normalizeSeverity, normalizeSnapshot, safeObject, stringify } from './rf-debug-center-state-utils.js'; import { clearCausalIntelligenceSnapshot as clearCausalIntelligenceSnapshotCore, copyCausalIntelligenceJSON as copyCausalIntelligenceJSONCore, getCausalIntelligenceSnapshot as getCausalIntelligenceSnapshotCore, refreshCausalIntelligenceSnapshot as refreshCausalIntelligenceSnapshotCore } from './rf-debug-center-causal-intelligence.js'; import { clearDomScannerSnapshot as clearDomScannerSnapshotCore, copyDomScannerJSON as copyDomScannerJSONCore, getDomScannerSnapshot as getDomScannerSnapshotCore, isDomScannerPaused as isDomScannerPausedCore, refreshDomScannerSnapshot as refreshDomScannerSnapshotCore } from './rf-debug-center-dom-scanner.js';
const MAX_RECENT = 12;
const timelineState = {
  paused: false,
  cursor: 0,
  ...
  warnings: { status: 'unknown', total: 0, counts: { info: 0, warning: 0, error: 0 }, warnings: [], sourceState: 'absent', traceState: 'absent' }, causalIntelligence: { status: 'unknown', summary: { bugsSuspected: 0, critical: 0, warnings: 0, unknown: 0, evidenceChains: 0 }, diagnoses: [], invariants: [], evidenceChains: [], ownershipViolations: [], unknowns: [], confidence: { overall: 'unknown', reason: 'no data' }, recommendations: [], limits: { maxDiagnoses: 50, maxEvidencePerDiagnosis: 12, maxChains: 30 } },
};
...
export function readDebugCenterState({ traceApi = window.RF_UI_TRACE, enabled = false, activation = 'disabled', bundle = null } = {}) {
  ...
  const renderPreview = refreshRenderPreviewSnapshot({ ds: typeof DS !== 'undefined' ? DS : null, doc: document, timeline, network, performance, asyncRace, loopFreeze, bundle, traceState: timeline.sourceState, active: Boolean(enabled) }); const domScanner = isDomScannerPausedCore() ? getDomScannerSnapshotCore() : refreshDomScannerSnapshotCore({ ds: typeof DS !== 'undefined' ? DS : null, doc: document, timeline, traceState: timeline.sourceState, bundle, ownership: window.RFDebugCenter?.ownership || null, active: Boolean(enabled) }); const causalIntelligence = refreshCausalIntelligenceSnapshotCore({ timeline, zoom, dom: { status: zoom.status || 'unknown', findings: zoom.divergences?.map((code) => ({ code })) || [], owners: zoom.suggestedOwner ? [zoom.suggestedOwner] : [] }, domScanner, loopFreeze, asyncRace, network, performance, selection, renderPreview, visualEvidence: getVisualEvidenceSnapshot(), bundle, ownership: window.RFDebugCenter?.ownership || null, now: new Date().toISOString() });
  timelineState.causalIntelligence = causalIntelligence;
  if (!timelineState.warningsPaused) timelineState.warnings = buildWarningsSnapshot({ traceState: timeline.sourceState, timeline, zoom, renderPreview, asyncRace, loopFreeze, network, performance, selection, visualEvidence: getVisualEvidenceSnapshot(), dom: { status: zoom.status || 'unknown', findings: zoom.divergences?.map((code) => ({ code })) || [], owners: zoom.suggestedOwner ? [zoom.suggestedOwner] : [] }, domScanner, causalIntelligence, bundle, ownership: window.RFDebugCenter?.ownership || null, active: Boolean(enabled) });
  return { enabled: Boolean(enabled), activation, build, debugZoom, last, live: normalizeSnapshot(live), divergence, zoom, timeline, selection, renderPreview, domScanner, causalIntelligence, asyncRace, loopFreeze, network, performance, warnings: timelineState.warnings, recent: timeline.recent, visualEvidence: getVisualEvidenceSnapshot() };
}
...
export function refreshDebugCenterDomScanner(traceApi = window.RF_UI_TRACE, bundle = null) { return refreshDomScannerSnapshotCore({ ds: typeof DS !== 'undefined' ? DS : null, doc: document, traceApi, timeline: getTimelineSnapshot(), traceState: timelineState.sourceState, bundle, ownership: window.RFDebugCenter?.ownership || null, active: true }); } export function clearDebugCenterDomScanner() { return clearDomScannerSnapshotCore(); } export function copyDebugCenterDomScannerJSON() { return copyDomScannerJSONCore(); } export function getDebugCenterDomScannerSnapshot() { return getDomScannerSnapshotCore(); } export function refreshDebugCenterCausalIntelligence(traceApi = window.RF_UI_TRACE, bundle = null) { syncTimeline(traceApi); return refreshCausalIntelligenceSnapshotCore({ timeline: getTimelineSnapshot(), zoom: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline: getTimelineSnapshot(), doc: document }), dom: { status: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline: getTimelineSnapshot(), doc: document }).status || 'unknown', findings: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline: getTimelineSnapshot(), doc: document }).divergences?.map((code) => ({ code })) || [], owners: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline: getTimelineSnapshot(), doc: document }).suggestedOwner ? [buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline: getTimelineSnapshot(), doc: document }).suggestedOwner] : [] }, domScanner: getDomScannerSnapshotCore(), loopFreeze: getLoopFreezeSnapshot(), asyncRace: getAsyncRaceSnapshot(), network: getNetworkSnapshot(), performance: getPerformanceSnapshot(), selection: getSelectionSnapshot(), renderPreview: getRenderPreviewSnapshot(), visualEvidence: getVisualEvidenceSnapshot(), bundle, ownership: window.RFDebugCenter?.ownership || null, now: new Date().toISOString() }); } export function clearDebugCenterCausalIntelligence() { return clearCausalIntelligenceSnapshotCore(); } export function copyDebugCenterCausalIntelligenceJSON() { return copyCausalIntelligenceJSONCore(); } export function getDebugCenterCausalIntelligenceSnapshot() { return getCausalIntelligenceSnapshotCore(); }
export function refreshDebugCenterWarnings(traceApi = window.RF_UI_TRACE, bundle = null) { timelineState.warningsPaused = false; const timeline = getTimelineSnapshot(); const selection = refreshSelectionSnapshot({ ds: typeof DS !== 'undefined' ? DS : null, doc: document, timeline, traceState: timeline.sourceState, bundle, ownership: window.RFDebugCenter?.ownership || null }); const asyncRace = refreshAsyncRaceSnapshot({ timeline, traceState: timeline.sourceState, bundle, active: true }); const loopFreeze = refreshLoopFreezeSnapshot({ timeline, traceState: timeline.sourceState, bundle, warnings: timelineState.warnings, active: true }); const network = getNetworkSnapshot(); const performance = refreshPerformanceSnapshot({ timeline, network, loopFreeze, asyncRace, warnings: timelineState.warnings, bundle, active: true, traceState: timeline.sourceState, win: window }); const renderPreview = refreshRenderPreviewSnapshot({ ds: typeof DS !== 'undefined' ? DS : null, doc: document, timeline, network, performance, asyncRace, loopFreeze, bundle, traceState: timeline.sourceState, active: true }); const domScanner = isDomScannerPausedCore() ? getDomScannerSnapshotCore() : refreshDomScannerSnapshotCore({ ds: typeof DS !== 'undefined' ? DS : null, doc: document, timeline, traceState: timeline.sourceState, bundle, ownership: window.RFDebugCenter?.ownership || null, active: true }); const causalIntelligence = refreshCausalIntelligenceSnapshotCore({ timeline, zoom: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline, doc: document }), dom: { status: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline, doc: document }).status || 'unknown', findings: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline, doc: document }).divergences?.map((code) => ({ code })) || [], owners: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline, doc: document }).suggestedOwner ? [buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline, doc: document }).suggestedOwner] : [] }, domScanner, causalIntelligence: timelineState.causalIntelligence, asyncRace, loopFreeze, network, performance, selection, renderPreview, visualEvidence: getVisualEvidenceSnapshot(), bundle, ownership: window.RFDebugCenter?.ownership || null, now: new Date().toISOString() }); return timelineState.warnings = buildWarningsSnapshot({ traceState: timelineState.sourceState, timeline, zoom: buildZoomDiagnostics({ ds: typeof DS !== 'undefined' ? DS : null, traceApi, timeline, doc: document }), renderPreview, asyncRace, loopFreeze, network, performance, selection, domScanner, causalIntelligence, bundle, ownership: window.RFDebugCenter?.ownership || null, active: true }); }
