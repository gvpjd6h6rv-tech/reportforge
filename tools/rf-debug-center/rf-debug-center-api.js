...
import { applyVisualEvidenceApi } from './rf-debug-center-api-visual-evidence.js';
...
const OWNERSHIP_MAP = Object.freeze({
  ...
  ssot: {
    ...
    performance: 'tools/rf-debug-center/rf-debug-center-performance.js', selection: 'tools/rf-debug-center/rf-debug-center-selection.js',
  },
  subsystems: [
...
export function createDebugCenterApi() {
  ...
  applySelectionApi(api, state, applyModel); applyRenderPreviewApi(api, state, applyModel); applyVisualEvidenceApi(api, state, applyModel);
  return { state, api };
}
