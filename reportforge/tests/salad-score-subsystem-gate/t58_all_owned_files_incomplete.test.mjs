...
test('T58: allOwnedFiles omitting a real partition file, or a partition file never declared in allOwnedFiles, both fail SCOPE-COMPLETENESS-01 (independent-source check, not a derived tautology)', async () => {
  const config = JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8'));
  await withMaterializedFixture({
    files: [SCENARIO.ownedCleanA, SCENARIO.ownedCleanB],
    scopeMap: { subsystems: { 'SS-INCOMPLETE': {
      files: ['engines/owned_clean_a.mjs', 'engines/owned_clean_b.mjs'],
      allOwnedFiles: ['engines/owned_clean_a.mjs'],
      allowedOwners: ['owner-x', 'owner-y'],
    } } },
    ownershipMap: { subsystems: [{ owner: 'owner-x', allowedFiles: ['owned_clean_a.mjs'] }, { owner: 'owner-y', allowedFiles: ['owned_clean_b.mjs'] }] },
  }, async (root) => {
    const result = runSubsystemGate({ root, config, ownershipMapPath: root + '/audit/subsystem_ownership_map.json', scopeMapPath: root + '/audit/subsystem_scope_map.json', subsystemId: 'SS-INCOMPLETE' });
