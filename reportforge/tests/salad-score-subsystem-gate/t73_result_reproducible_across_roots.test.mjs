...
test('T73: the SAME subsystem materialized in two DIFFERENT temp roots (A, B) produces byte-identical stdout JSON -- no root path leaks into any field', async () => {
  const config = JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8'));
  const runOnce = (root) => runSubsystemGate({ root, config, ownershipMapPath: root + '/audit/subsystem_ownership_map.json', scopeMapPath: root + '/audit/subsystem_scope_map.json', subsystemId: 'SS-VALID' });
  const resultA = await withMaterializedFixture(DESCRIPTOR, async (root) => runOnce(root));
  const resultB = await withMaterializedFixture(DESCRIPTOR, async (root) => runOnce(root));
