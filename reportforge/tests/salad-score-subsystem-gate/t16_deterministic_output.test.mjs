...
test('T16: running the gate twice on the same scope produces byte-identical output', async () => {
  ...
  await withMaterializedFixture({
    ...
    const args = { root, config, ownershipMapPath: root + '/audit/subsystem_ownership_map.json', scopeMapPath: root + '/audit/subsystem_scope_map.json', subsystemId: 'SS-VALID' };
    const a = JSON.stringify(runSubsystemGate(args));
    const b = JSON.stringify(runSubsystemGate(args));
    assert.equal(a, b);
  });
});
