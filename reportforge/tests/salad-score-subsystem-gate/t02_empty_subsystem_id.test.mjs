...
test('T2: an empty subsystem ID fails the gate, never a false green', async () => {
  ...
  await withMaterializedFixture({
    ...
    const result = runSubsystemGate({ root, config, ownershipMapPath: root + '/audit/subsystem_ownership_map.json', scopeMapPath: root + '/audit/subsystem_scope_map.json', subsystemId: '' });
