...
test('T3: a subsystem ID absent from the scope map fails the gate', async () => {
  ...
  await withMaterializedFixture({
    ...
    const result = runSubsystemGate({ root, config, ownershipMapPath: root + '/audit/subsystem_ownership_map.json', scopeMapPath: root + '/audit/subsystem_scope_map.json', subsystemId: 'SS-DOES-NOT-EXIST' });
    assert.equal(result.FINAL_GATE_STATUS, 'FAIL');
    assert.equal(result.CHECKS.subsystemIdKnown, 'FAIL');
  });
});
