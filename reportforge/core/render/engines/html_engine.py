...
class HtmlEngine:
    ...
    def __init__(self, layout: Layout, resolver: FieldResolver, debug: bool = False):
        self._layout = layout
...
"""
    ...
    def _barcode_font_css(self) -> str:
        ...
        for el in self._layout.elements:
            ...
            if source["kind"] == "path":
                faces.append(
                    f"@font-face{{font-family:'{family}';src:local('{family}'),"
                    f"url('{source['url']}') format('{source['format']}');}}"
                )
            else:
    ...
    def _resolve_barcode_font_source(self, el, family: str) -> dict[str, str] | None:
        ...
        layout_path = str(getattr(el, "barcodeFontPath", "") or getattr(self._layout, "barcodeFontPath", "") or "")
        ...
        if self._fc_exact_family_exists(family):
    ...
    def _render_section(self, sec: Section) -> str:
        ...
        return (
            f'<div class="cr-section" data-section="{sec.id}" data-stype="{sec.stype}" '
            f'style="height:{sec.height}px">{inner}</div>'
        )
    ...
    def _render_detail_section(self, sec: Section) -> str:
        ...
        for idx, item in enumerate(items):
            resolver = self._resolver.with_item(item)
            alt = idx % 2 == 1
            row_bg = f"background:{self.ROW_EVEN}" if alt else f"background:{self.ROW_ODD}"
            inner = "".join(self._render_element(el, resolver) for el in elements)
            rows.append(
                f'<div class="cr-detail-row" style="height:{sec.height}px;{row_bg}">{inner}</div>'
            )
        return "\n".join(rows)

    def _render_element(self, el: Element, resolver: Optional[FieldResolver] = None) -> str:
    ...
    def _render_field(self, el: Element, r: FieldResolver) -> str:
        ...
        return f'<div class="cr-el" style="{style}"><span class="cr-el-inner">{value}</span></div>'
    ...
    def _render_line(self, el: Element) -> str:
        ...
        style = (
            f"position:absolute;left:{el.x}px;top:{el.y}px;width:{el.w}px;height:{max(el.h, lw)}px;"
            f"overflow:visible"
        )
        if el.lineDir == "v":
            svg = (
    ...
    def _render_rect(self, el: Element) -> str:
        ...
        style = (
            ...
            f"background:{bg};{brd}box-sizing:border-box;z-index:{el.zIndex}"
        )
    ...
    def _render_image(self, el: Element) -> str:
        style = (
            f"position:absolute;left:{el.x}px;top:{el.y}px;width:{el.w}px;height:{el.h}px;"
            f"border:1px dashed #AAA;background:#F5F5F5;display:flex;align-items:center;justify-content:center;"
