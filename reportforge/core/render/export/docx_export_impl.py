...
def export_docx(layout_raw: dict, data: dict, output_path: str | Path) -> Path:
    try:
    ...
    def _add_section_para(sec_elements: list, item: dict | None = None, gitems: list | None = None, bold_override: bool = False) -> None:
        texts = []
    ...
    det_els_flat = [el for s in det_secs for el in sec_map.get(s["id"], []) if el.get("type") in ("text", "field") and el.get("fieldPath")]
    ...
    if det_els_flat:
        ...
        if not groups:
            ...
            for gval, git in _groupby(sorted_items, key=lambda it: it.get(gf_field, "")):
                gitems = list(git)
                gh_els = [el for s in gh_secs for el in sec_map.get(s["id"], []) if el.get("type") in ("text", "field")]
                if gh_els:
                    gr = tbl.add_row()
                ...
                gf_els = [el for s in gf_secs for el in sec_map.get(s["id"], []) if el.get("type") in ("text", "field")]
                if gf_els:
                    gfr = tbl.add_row()
