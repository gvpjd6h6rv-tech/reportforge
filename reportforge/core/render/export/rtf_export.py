# core/render/export/rtf_export.py
# RTF export — Phase 4
# Generates valid RTF 1.9 without external dependencies.
# Pure Python string assembly: header, color table, font table, body.
from __future__ import annotations
from pathlib import Path


def export_rtf(layout_raw: dict, data: dict,
               output_path: str | Path) -> Path:
    """Render a ReportForge layout to RTF 1.9 format."""
    ...
    colortbl = (
        ...
        r"\red192\green81\blue26;"      # 3: brand orange
        r"\red26\green58\blue107;"      # 4: brand blue
        r"\red100\green100\blue100;"    # 5: gray
        r"}"
    )
    ...
    # Detail section as a simple table
    det_secs = secs_by_type.get("det", [])
    det_els  = [e for s in det_secs for e in sec_map.get(s["id"],[])
                if e.get("type") in ("text","field") and e.get("fieldPath")]
    det_els.sort(key=lambda e: e.get("x",0))

    groups = norm.get("groups",[])

    if det_els:
        col_w = max(1000, 8000 // max(1, len(det_els)))  # twips
        # Table header
