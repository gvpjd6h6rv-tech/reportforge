...
// file → [subsystem ids that claim it]
...
// RULE-EXIST: every allowedFile must exist on disk
for (const ss of subsystems) {
  // Only check designer-runtime subsystems that have allowedFiles
...
for (const f of diskFiles) {
  ...
  // Overlap in sharedFiles is already allowed; double-claim without sharedFiles = RULE-OVERLAP above
}
...
for (const ss of subsystems) {
  ...
  // domain must be one of the two valid values
  ...
  // tier must be core or supporting
