...
| RF-ARCH-007 | Remove DOM aliases | duplicate DOM aliases and legacy IDs keep duplicate contract alive | monolith, QA scripts | medio | alto | P1 | no runtime alias injection remains |
...
| RF-ARCH-011 | Define canonical DOM contract | `.cr-element` vs alternate modular contract, `#handles-layer` vs `#sel-layer` | monolith, modular runtime, docs | medio | alto | P1 | one DOM contract document and one live implementation |
